...
roi_dict_MNI_sg_subcort = {# 'L-CN': 1, 'L-SOC': 3, 
                           'L-IC': 5, 'L-MGN': 7, 
                           #'R-CN': 2, 'R-SOC': 4, 
                           'R-IC': 6, 'R-MGN': 8, }
...
model_sub_dir = os.path.join(glmsingle_dir, 
                             'subject_level',
                             f'sub-{subject_id}', 
                             'beta_images'
                              )
...
for mx, mask_descrip in enumerate(roi_list):
    ...
    if window_descrip=='trial':
        ...
        out_dir = os.path.join(glmsingle_dir,
                               'masked_statmaps',
                               f'sub-{subject_id}', 
                               'statmaps_masked',
                               f'mask-{mask_descrip}')
        ...
        for sx, stat_fpath in enumerate(stat_maps):
            ...
            try:
                ...
                print('Cannot mask - is empty')

            # save out
            out_fbase = f'{cond_label}_mask-{mask_descrip}'

            out_fpath = os.path.join(out_dir, out_fbase+'.csv')
            try:
                np.savetxt(out_fpath, fmri_masked)
            except ValueError:
                print('Cannot save to csv - Expected 1D or 2D array, got 0D array instead')
